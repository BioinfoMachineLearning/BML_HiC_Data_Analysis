...
def create_bed(chrom='all', bin_size=50000000, step_size=10000):
    '''
    ...
    if chrom == 'all':
        bed_path = bed_path.format('full', convert_bytes(bin_size).replace(" ", ""), convert_bytes(step_size).replace(" ", ""))
    else:
        bed_path = bed_path.format(chrom, convert_bytes(bin_size).replace(" ", ""), convert_bytes(step_size).replace(" ", ""))

    if not os.path.isfile(bed_path):
        print('Creating Bed file {} '.format(bed_path))

        if chrom == 'all':
            subprocess.call('chess pairs mm10 {} {} {}'.format(bin_size, step_size, bed_path), shell=True)
        else:
            subprocess.call('chess pairs mm10  {} {} {} --chromosome {}'.\
                            format(bin_size, step_size, bed_path, chrom), shell=True)

    else:
        print("Bed file {} exist".format(bed_path))


def search_similarity(bin_size, step_size, pair_1, pair_2):
    '''
    ...
    for i in chros:
        create_bed(i, bin_size, step_size)
        chess_sim_results = '../Results/SIM/{}_{}_{}_vs_{}_chess_results.tsv'.format(i, convert_bytes(bin_size).replace(" ", ""), pair_1, pair_2)
        if not os.path.isfile(chess_sim_results):
            ...
            subprocess.call('chess sim \
                                        ...
                                        {} \
                                        -p 4'.format(pair_1, pair_2, i, convert_bytes(bin_size).replace(" ", ""),
        ...
        if not os.path.isfile(chess_extract_results):
            ...
            subprocess.call('chess extract \
                                        ...
                                        ../MuSC_HiC_files/HiC_CPB_normalized/{}_new_CPBnorm_inter_30.hic \
                                        {} \
                                        '.format(i, convert_bytes(bin_size).replace(" ", ""),
...
def examine_similarity(bin_size=50000000, step_size=10000):
    for i in pairs:
        search_similarity(bin_size, step_size, i[0].upper(), i[1].upper())


def vis_sim_results(bin_size=50000000, step_size=10000, resolution=1000000, features=False, save_fig=False):
    sn_thr = 0.5
    sim_field = "z_ssim"
    sn_thr = sn_thr
    zsim_thr = -1
    ...
    for chrom in chros:
        for pair in pairs:
            regions = pd.read_csv('../Results/Bed/mm10_{}_{}_win_{}_step.bed'. \
                                  format(chrom, convert_bytes(bin_size).replace(" ", ""), convert_bytes(step_size).replace(" ", "")), sep='\t', header=None)
            similarities = pd.read_csv('../Results/SIM/{}_{}_{}_vs_{}_chess_results.tsv'. \
                                  format(chrom, convert_bytes(bin_size).replace(" ", ""), pair[0].upper(), pair[1].upper()), sep='\t', index_col=0)
...
# search_similarity()
# examine_similarity(bin_size=50000000, step_size=10000)
